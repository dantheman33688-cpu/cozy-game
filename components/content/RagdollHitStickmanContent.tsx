// components/content/RagdollHitStickmanContent.tsx
'use client';

import React from 'react';
import Image from 'next/image';

interface RagdollHitStickmanContentProps {
  content: {
    title: string;
    sections: any[];
  };
...
export default function RagdollHitStickmanContent({ content }: RagdollHitStickmanContentProps) {
  ...
  const renderSection = (section: any) => {
    ...
    if (section.type === 'intro') {
      return (
        <div
          id={section.id}
          className="bg-white rounded-lg shadow-md p-6 mb-6 scroll-mt-20"
        >
    ...
    if (section.type === 'conclusion') {
      return (
        <div
          id={section.id}
          className="bg-gradient-to-r from-green-50 to-blue-50 rounded-lg shadow-md p-6 mb-6 border-l-4 border-green-500 scroll-mt-20"
        >
    ...
    return (
      ...
      >
        ...
        {section.subsections?.map((sub: any, index: number) => (
          <div key={index}>{renderSubSection(sub)}</div>
        ))}
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4 max-w-4xl">
        {/* 顶部标题区块 - 使用绿色到蓝色的渐变（活泼有趣的主题） */}
        <div className="bg-gradient-to-r from-green-500 to-blue-600 text-white rounded-lg shadow-lg p-8 mb-8">
          <div className="text-center">
            <h2 className="text-4xl font-bold mb-4 leading-tight">{content.title}</h2>
          </div>
        </div>
